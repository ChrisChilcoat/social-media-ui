...
export default function FbHeader() {
  ...
  const initalDelay = 1000 // Keeps tooltips from becoming annaying.
  const secondaryDelay = 0 // Speed up tooltips once they are open.

  const [delay, setDelay] = useState(initalDelay)
        ...
        <div className="flex items-center justify-between py-1">
          ...
          <div className="flex-1">
            <div className="max-w-2xl m-auto">
              <ul  aria-label="Other Pages of Interest" role="navigation" className="grid gap-0 -mb-1 lg:gap-2 lg:px-2 grid-col-1 sm:grid-cols-5 md:grid-cols-5 lg:grid-cols-5">                
                {navigation.map((item, index) => (
                  <Tippy content={item.name} delay={delay}>
                  <li
                    className={
          ...
          <div aria-label="Account Controls and Settings" role="navigation" className="flex items-center justify-end w-4/12 md:w-3/12">
            ...
            {notifications.map((item, index) => (
              <Tippy content={item.name} delay={delay}>
                <button 
                  aria-label={item.name + ' , ' + item.notifications + ' unread'} 
                  id={item.name} 
                  className="relative flex-shrink-0 block p-2.5 m-1 text-gray-900 bg-gray-200 rounded-full hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2 focus:ring-opacity-60">
                  <item.icon
                    className="w-5 h-5"
                    aria-hidden="true"
                    fill="currentColor"
