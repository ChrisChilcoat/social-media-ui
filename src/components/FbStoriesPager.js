...
function FbStoriesPager() {
  ...
  const toggleNextBtn = () => {
    ...
    totalWidth == currentScrollLeft ? setNextBtn(false) : setNextBtn(true);
  }
  ...
  const AddBtn = () => (
    <button style={addWrapperStyles} class={ (scrollPosition < 6 || scrollPosition == null ? 'hidden' : null) + " overflow-hidden transform -translate-y-1/2 top-1/2 transform-gpu -left-5 top-1 w-1/5 z-30 absolute items-center ml-5 rounded-xl shadow border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2 focus:ring-opacity-60"}>
      <img style={addImageStyles} class="max-h-40 max-w-40 m-auto not-sr-only object-cover w-full h-4/5 relative transition-transform transform-gpu opacity-80 hover:opacity-75 group-focus:opacity-75 hover:scale-102 group-focus:scale-102" 
      ...
      <div className="relative z-50 bg-white group-hover:bg-gray-100 h-1/5">
        <svg style={addPlusStyles} xmlns="http://www.w3.org/2000/svg" class="absolute m-h-8 m-w-8 text-white bg-blue-500 block rounded-full mx-auto absolute bottom-1/3 left-1/2 -ml-4 -top-4 border-4 border-white" fill="none" viewBox="-3 -3 30 30" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
        </svg>
        <div style={addTextStyles} class="text-gray-700 block m-auto pt-4 text-xs font-medium">Create Story</div>
      </div>
    </button>
  )

  return (
    <section aria-label="View Stories" role="region" class="relative max-w-2xl mx-auto mb-5 mt-5">
      <AddBtn/>
      <div ref={pager} onScroll={update} class="relative w-full overflow-x-auto scrollbar-hide">
        <ul className="whitespace-nowrap">
          <li className={ (scrollPosition > 5 ? 'invisible' : null) + " align-top relative inline-block w-1/4 sm:w-1/5"}>
            <button class="relative m-1 group overflow-hidden shadow transition-transform bg-black rounded-xl group h-52 transform-gpu active:scale-98 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2 focus:ring-opacity-60 z-10">
              <img class="not-sr-only object-cover w-full h-4/5 relative transition-transform transform-gpu opacity-80 hover:opacity-75 group-focus:opacity-75 hover:scale-102 group-focus:scale-102" 
                src='https://images.unsplash.com/photo-1463453091185-61582044d556?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facearea&facepad=8&w=1024&h=1024&q=80'
              ></img>
              <div className="relative z-50 bg-white group-hover:bg-gray-100 h-1/5">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-white bg-blue-500 block rounded-full mx-auto absolute bottom-1/3 left-1/2 -ml-4 -top-4 border-4 border-white" fill="none" viewBox="-3 -3 30 30" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
                </svg>
          ...
          {stories.map((story) => 
            <li key={story.name} className="relative inline-block w-1/4 sm:w-1/5">
              <button className="relative m-1 overflow-hidden transition-transform bg-black shadow rounded-xl group h-52 transform-gpu active:scale-98 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2 focus:ring-opacity-60">
                <img class="not-sr-only object-cover w-full h-full  relative transition-transform transform-gpu opacity-80 hover:opacity-75 group-focus:opacity-75 hover:scale-102 group-focus:scale-102" 
                  src={story.imageUrl}
                  alt={story.name}
                ></img>
